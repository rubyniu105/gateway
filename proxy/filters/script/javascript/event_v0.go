...
func newBeatEventV0Constructor(s Session) func(call goja.ConstructorCall) *goja.Object {
	return func(call goja.ConstructorCall) *goja.Object {
		...
		evt.init()
		evt.reset(&fasthttp.RequestCtx{Parameters: param.Parameters{Data: fields}}) //TODO
		return nil
	}
}
...
func (e *beatEventV0) reset(b *fasthttp.RequestCtx) error {
	e.inner = b
	e.cancelled = false
	e.obj.Set("_private", e)
	e.obj.Set("fields", e.vm.ToValue(e.inner.Data)) //access fields for none-context only
	return nil
}

// Wrapped returns the wrapped beat.Event.
func (e *beatEventV0) Wrapped() *fasthttp.RequestCtx {
	return e.inner
}
...
//	 // javascript
//		var dataset = evt._ctx.field1.field2;
func (e *beatEventV0) getContext(call goja.FunctionCall) goja.Value {
	a0 := call.Argument(0)
	if goja.IsUndefined(a0) {
		// event.Get() is the same as event.fields (but slower).
		return e.vm.ToValue(e.inner.Data)
	}

	log.Errorf("start get ctx", a0.ToString())

	v, err := e.inner.GetValue("_ctx." + a0.String())
	if err != nil {
		return goja.Null()
	}
...
func (e *beatEventV0) tag(call goja.FunctionCall) goja.Value {
	...
	if err := appendString(e.inner.Data, "tags", tag, true); err != nil {
		panic(err)
	}
	return goja.Undefined()
}
...
func (e *beatEventV0) appendTo(call goja.FunctionCall) goja.Value {
	...
	if err := appendString(e.inner.Data, field, value, false); err != nil {
		panic(err)
	}
	return goja.Undefined()
}

func appendString(m util.MapStr, field, value string, alwaysArray bool) error {
